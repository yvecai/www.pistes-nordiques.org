...
function refreshOverlay(features) {
	...
	for (i=0;i<features.length;i++){ 
		if (features[i].attributes['route']){}
			...
			if (features[i].attributes['piste:type']=='nordic'){
...
var pointStyle = new OpenLayers.Style(
				...
				{context: {
					 ...
					 color: function(feature) {
							if (feature.attributes['type'] == 'nordic') {return '#FFCF00'}
							else {return '#0060FF'}
						}
					}
				});
...
var styleContext = {
	getColor: function(feature) {
		...
		if ( feature.attributes['piste:type'] == 'downhill' ) {
			switch(feature.attributes['piste:difficulty']){
				...
				case 'freeride': return 'yellow';
			}
		}
		if ( feature.attributes['aerialway'] != null ) {return 'black';}
		else {
	...
	getOpacity: function(feature) {
		if ( feature.attributes['piste:type'] == 'downhill' ) {return 0.4;}
		else {return 0.9;}
	},
	getZ: function(feature) {
		if (feature.attributes['route_name']!= null) {return 12;}
		else {return 11;}
	},
	...
	getWidth: function(feature) {
		...
		if ( feature.attributes['piste:type'] == 'downhill' ) {return 5;}
		if ( feature.attributes['aerialway'] != null ) {return 1;}
		else {
			if (feature.attributes['route_name']!= null) {return 2;}
			else {return 3;}
		}
	},
...
OpenLayers.ProxyHost = server+"cgi/proxy.cgi?url=";
	...
	var pistesLayerLowres = new OpenLayers.Layer.Vector("Pistes Vector LR", {
		...
		protocol: new OpenLayers.Protocol.HTTP({
				url:server+"apistes?simplify=true&",
	...
	var pistesLayer = new OpenLayers.Layer.Vector("Pistes Vector", {
		...
		styleMap: pisteStyles,
		protocol: new OpenLayers.Protocol.HTTP({
				url:server+"apistes?simplify=false&",
				format: new OpenLayers.Format.GeoJSON()
		}),
		minScale: 200000, //12
		visibility: true,
